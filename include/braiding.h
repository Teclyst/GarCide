...
using CBraid::BandBraid;
using CBraid::BandFactor;
using CBraid::sint16;
using std::cout;
using std::endl;
using std::list;
...
template <class P> sint16 CL(CBraid::Braid<P> B) {
    sint16 n = 0;
...
template <class P> sint16 Sup(CBraid::Braid<P> B) {
    sint16 s;
...
template <class P> CBraid::Braid<P> Cycling(CBraid::Braid<P> B) {
...
template <class P> CBraid::Braid<P> Decycling(CBraid::Braid<P> B) {
    sint16 n;
...
template <class P> CBraid::Braid<P> WordToBraid(list<sint16> w, sint16 n) {
    ...
    for (it = w.begin(); it != w.end(); it++) {
        if (*it > 0) {
            ...
            if (*it == -n) {
                ...
                F[i + 1] = k;
                F = (~F).Flip();
                B2 = (!CBraid::Braid<P>(CBraid::Factor<P>(n, 1))) *
                     CBraid::Braid<P>(F);
            }
            B.RightMultiply(B2);
        }
...
template <class P> void PrintBraidWord(CBraid::Braid<P> B) {
    if (B.LeftDelta == 1) {
        cout << "D";
        if (CL(B))
            cout << " . ";
...
template <class P> void PrintBraidWord(CBraid::Braid<P> B, char *file) {
...
CBraid::Braid<P> LeftMeet(CBraid::Braid<P> B1, CBraid::Braid<P> B2) {
    ...
    CBraid::Braid<P> B = CBraid::Braid<P>(n);
    CBraid::Factor<P> F1 = CBraid::Factor<P>(n, 0),
                      F2 = CBraid::Factor<P>(n, 0), F = CBraid::Factor<P>(n, 1);
...
template <class P> list<CBraid::Factor<P>> MinSSS(CBraid::Braid<P> B) {
    sint16 i, j, k, test;
    sint16 n = B.Index();
...
template <class P> list<CBraid::Braid<P>> SSS(CBraid::Braid<P> B) {
...
template <class P> list<CBraid::Braid<P>> Trajectory(CBraid::Braid<P> B) {
...
template <class P> CBraid::Braid<P> SendToUSS(CBraid::Braid<P> B) {
    CBraid::Braid<P> B2 = SendToSSS(B);
...
/////////////////////////////////////////////////////////////
//
//  Transport(B,F)   Given a braid B (in its USS and in LCF), and a simple
//  factor
...
CBraid::Factor<P> Transport(CBraid::Braid<P> B, CBraid::Factor<P> F) {
    ...
    CBraid::Braid<P> B3 = ((!CBraid::Braid<P>(*B.FactorList.begin())) * F *
                           (*B2.FactorList.begin()))
                              .MakeLCF();
...
//                 transports of F that send B to an element in the trayectory
...
template <class P> list<CBraid::Factor<P>> MinUSS(CBraid::Braid<P> B) {
    sint16 i, j, k, test;
    sint16 n = B.Index();
    sint16 *table = new sint16[n];
...
template <class P> list<list<CBraid::Braid<P>>> USS(CBraid::Braid<P> B) {
    ...
    B2 = ((!CBraid::Braid<P>(CBraid::Factor<P>(B.Index(), 1))) *
          (Cycling(*rit)) * CBraid::Factor<P>(B.Index(), 1))
             .MakeLCF();
    ...
    while (it != uss.end()) {
        ...
        for (itf = Min.begin(); itf != Min.end(); itf++) {
            ...
            if (itb == T.end()) {
                ...
                B2 = ((!CBraid::Braid<P>(CBraid::Factor<P>(B.Index(), 1))) *
                      (*T.begin()) * CBraid::Factor<P>(B.Index(), 1))
                         .MakeLCF();
                for (itb = T.begin(); itb != T.end(); itb++) {
                    if (B2 == *itb)
                        break;
...
//                    and stores in the lists 'mins' and 'prev' the following
//                    data: for each i, the first braid of the orbit i is
//                    obtained by conjugation of the first element of the orbit
//                    prev[i] by the simple element mins[i].
//
/////////////////////////////////////////////////////////////

template <class P>
list<list<CBraid::Braid<P>>>
USS(CBraid::Braid<P> B, list<CBraid::Factor<P>> &mins, list<sint16> &prev) {
    list<list<CBraid::Braid<P>>> uss;
...
template <class P>
bool AreConjugate(CBraid::Braid<P> B1, CBraid::Braid<P> B2,
                  CBraid::Braid<P> &C) {
    sint16 n = B1.Index();
...
list<CBraid::Braid<P>> Centralizer(list<list<CBraid::Braid<P>>> &uss,
    ...
    typename list<list<CBraid::Braid<P>>>::iterator it;
    typename list<CBraid::Braid<P>>::iterator itb;
    CBraid::Braid<P> C = CBraid::Braid<P>(n), D = CBraid::Braid<P>(n),
                     E = CBraid::Braid<P>(n), B2 = CBraid::Braid<P>(n);
    ...
    for (it = uss.begin(); it != uss.end(); it++) {
        ...
        for (itMin = Min.begin(); itMin != Min.end(); itMin++) {
            C = D * (*itMin);
            B2 = ((!CBraid::Braid<P>(*itMin)) * (*(*it).begin()) * (*itMin))
                     .MakeLCF();
            E = TreePath(B2, uss, mins, prev);
            C = C * (!E);
            C.MakeLCF();

            if (C.CompareWithIdentity() == false &&
                find(Cent.begin(), Cent.end(), C) == Cent.end())
                Cent.push_back(C);
        }
    }
...
template <class P> list<CBraid::Braid<P>> Centralizer(CBraid::Braid<P> B) {
    sint16 n = B.Index();
    list<CBraid::Factor<P>> mins;
    list<sint16> prev;
...
template <class P> void Tableau(CBraid::Factor<P> F, sint16 **&tab) {
    sint16 i, j;
    sint16 n = F.Index();
    for (i = 0; i < n; i++) {
        tab[i][i] = F[i + 1];
...
template <class P> bool Circles(CBraid::Braid<P> B) {
...
template <class P> int ThurstonType(CBraid::Braid<P> B) {
    sint16 i, n = B.Index();
...
template <class P> int ThurstonType(list<list<CBraid::Braid<P>>> &uss) {
    CBraid::Braid<P> B = *(*uss.begin()).begin();
    sint16 i, n = B.Index();
...
template <class P> sint16 Rigidity(CBraid::Braid<P> B) {
...
template <class P> sint16 Rigidity(list<list<CBraid::Braid<P>>> &uss) {
    typename list<list<CBraid::Braid<P>>>::iterator it;
...
template <class P> CBraid::Braid<P> RaisePower(CBraid::Braid<P> B, sint16 k) {
...
void PrintUSS(list<list<CBraid::Braid<P>>> &uss, list<sint16> word, sint16 n,
    ...
    f << "This file contains the Ultra Summit Set of the braid on " << n
      << " strands:" << endl
      << endl;
    if (power != 1)
        f << "( ";
    ...
    for (it = uss.begin(); it != uss.end(); it++) {
        ...
        if (((!CBraid::Braid<P>(CBraid::Factor<P>(n, 1))) * (*(*it).begin()) *
             CBraid::Braid<P>(CBraid::Factor<P>(n, 1)))
                .MakeLCF() == B2)
            ...
            for (itb = (*it).begin(); itb != (*it).end(); itb++) {
                if (((*(*it).begin()) *
                     CBraid::Braid<P>(CBraid::Factor<P>(n, 1)))
                        .MakeLCF() ==
                    (CBraid::Braid<P>(CBraid::Factor<P>(n, 1)) * (*itb))
                        .MakeLCF())
                    break;
            }
            if (itb != (*it).end())
...
/////////////////////////////////////////////////////////////
//
//   FileName(iteration,max_iteration,type,orbit,rigidity,cl)  Creates the file
//   name
...
template <class P> CBraid::Braid<P> Reverse(CBraid::Braid<P> B) {
    sint16 i, l = CL(B);
...
template <class P> CBraid::Factor<P> InitialFactor(CBraid::Braid<P> B) {
    sint16 n = B.Index();
...
template <class P> CBraid::Factor<P> PreferredPrefix(CBraid::Braid<P> B) {
...
template <class P> CBraid::Braid<P> Sliding(CBraid::Braid<P> B) {
    CBraid::Factor<P> F = CBraid::Factor<P>(B.Index());
...
template <class P> CBraid::Factor<P> PreferredSuffix(CBraid::Braid<P> B) {
...
/////////////////////////////////////////////////////////////
//
//  Trajectory_Sliding(B,C,d)  Computes the trajectory of a braid B for cyclic
//  sliding,
...
template <class P> CBraid::Braid<P> SendToSC(CBraid::Braid<P> B) {
...
/////////////////////////////////////////////////////////////
//
//  Transport_Sliding(B,F)   Given a braid B (in its SC and in LCF), and a
//  simple factor
//                   F such that B^F is in its SSS, computes the transport of F
//                   for sliding.
//
...
CBraid::Factor<P> Transport_Sliding(CBraid::Braid<P> B, CBraid::Factor<P> F) {
    CBraid::Braid<P> B2 = ((!CBraid::Braid<P>(F)) * B * F).MakeLCF();
    CBraid::Braid<P> B3 =
        ((!CBraid::Braid<P>(PreferredPrefix(B))) * F * (PreferredPrefix(B2)))
            .MakeLCF();
...
/////////////////////////////////////////////////////////////
//
//  Returns_Sliding(B,F)   Given a braid B (in its SC and in LCF), and a simple
//  factor
//                 F such that B^F is in its SSS, computes the iterated
//                 transports of F for sliding that send B to an element in the
//                 circuit of B^F.
//
/////////////////////////////////////////////////////////////

template <class P>
list<CBraid::Factor<P>> Returns_Sliding(CBraid::Braid<P> B,
                                        CBraid::Factor<P> F) {
    list<CBraid::Factor<P>> ret;
    typename list<CBraid::Factor<P>>::iterator it = ret.end();
    CBraid::Braid<P> B1 = B;
...
template <class P>
CBraid::Factor<P> MainPullback_Sliding(CBraid::Braid<P> B,
                                       CBraid::Factor<P> F) {
...
template <class P> list<CBraid::Factor<P>> MinSC(CBraid::Braid<P> B) {
    sint16 i, j, k, test;
    sint16 n = B.Index();
    sint16 *table = new sint16[n];
...
template <class P> list<list<CBraid::Braid<P>>> SC(CBraid::Braid<P> B) {
...
void PrintSC(list<list<CBraid::Braid<P>>> &sc, list<sint16> word, sint16 n,
    ...
    f << "This file contains the Set of Sliding Circuits of the braid on " << n
      << " strands:" << endl
      << endl;
    if (power != 1)
        f << "( ";
...
template <class P>
list<list<CBraid::Braid<P>>>
SC(CBraid::Braid<P> B, list<CBraid::Factor<P>> &mins, list<sint16> &prev) {
    list<list<CBraid::Braid<P>>> sc;
...
template <class P>
bool AreConjugateSC(CBraid::Braid<P> B1, CBraid::Braid<P> B2,
                    CBraid::Braid<P> &C) {
    sint16 n = B1.Index();
    CBraid::Braid<P> C1 = CBraid::Braid<P>(n), C2 = CBraid::Braid<P>(n);
...
template <class P>
bool AreConjugateSC2(CBraid::Braid<P> B1, CBraid::Braid<P> B2,
                     CBraid::Braid<P> &C) {
    list<list<CBraid::Braid<P>>> sc;
    sint16 n = B1.Index();
    CBraid::Factor<P> F = CBraid::Factor<P>(n);

...
namespace SSS
{
  ...
  template <class F>
  Braid<F> SendToSSS(const Braid<F> &b, const Braid<F> &c)
  {
  ...
  // The reason for doing it that way being to have function signatures that are closer to those for USSs and SCSs.
  // This might change later - it could be interesting to instead use a map and save for each element a conjugator.
  template <class B>
  class SuperSummitSet
  {
    std::unordered_set<B> Set;

  public:
    inline void Insert(B b)
    {
      Set.insert(b);
    }
    ...
    {
      for (typename std::unordered_set<B>::const_iterator it = Set.begin(); it != Set.end(); it++)
      {
        (*it).Print(os);
        os << std::endl;
      }
  ...
  template <class F>
  SuperSummitSet<Braid<F>> SSS(const Braid<F> &b)
  {

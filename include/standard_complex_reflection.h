...
namespace CGarside
{
  ...
  public:
    ...
    // Prints to `os` Neaime's shortest word representative in the Corran-Picantin generators (See George Neaime, Interval Garside Structures for the Complex Braid Groups, [arXiv:1707.06864](https://arxiv.org/abs/1707.06864))).
    // @param os The `std::ostream` we are printing to. 
    ...
    // Checks if s_i left divides `*this`. (See George Neaime, Interval Garside Structures for the Complex Braid Groups, Proposition 3.13, [arXiv:1707.06864](https://arxiv.org/abs/1707.06864)).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor.
    ...
    {
      return (PermutationTable[i - 1] > PermutationTable[i - 2]) ? (CoefficientTable[i - 1] != 0) : (CoefficientTable[i - 2] == 0);
    };

    // Checks if t_i left divides `*this`. (See George Neaime, Interval Garside Structures for the Complex Braid Groups, Proposition 3.13, [arXiv:1707.06864](https://arxiv.org/abs/1707.06864)).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor.
    ...
    {
      return (PermutationTable[1] > PermutationTable[0]) ? (CoefficientTable[1] != 0) : (CoefficientTable[0] == ((i == 0) ? 0 : GetParameter().e - i));
    };

    // Left multiplies by s_i (or divides, which is the same as it has order 2).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor. Modified by this function so that it remains up to date.
    // @param i The integer i for which we multiply by s_i left divides the factor.
    void SLeftMultiply(sint16 *dir_perm, sint16 i)
    {
      ...
      std::swap(dir_perm[PermutationTable[i - 1]], dir_perm[PermutationTable[i - 2]]);
    };

    // Left multiplies by t_i (or divides, which is the same as it has order 2).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor. Modified by this function so that it remains up to date.
    // @param i The integer i for which we multiply by t_i left divides the factor.
    ...
    // Right multiplies by s_i (or divides, which is the same as it has order 2).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor. Modified by this function so that it remains up to date.
    // @param i The integer i for which we multiply by s_i right divides the factor.
    void SRightMultiply(sint16 *dir_perm, sint16 i)
    {
      std::swap(PermutationTable[dir_perm[i - 1]], PermutationTable[dir_perm[i - 2]]);
      std::swap(dir_perm[i - 1], dir_perm[i - 2]);
    };

    // Right multiplies by t_i (or divides, which is the same as it has order 2).
    // @param dir_perm A table that holds the direct table of the permutation induced by the factor. Modified by this function so that it remains up to date.
    // @param i The integer i for which we multiply by t_i right divides the factor.
    void TRightMultiply(sint16 *dir_perm, sint16 i)
    {
      std::swap(PermutationTable[dir_perm[0]], PermutationTable[dir_perm[1]]);
      CoefficientTable[dir_perm[0]] = Rem(CoefficientTable[dir_perm[0]] - i, GetParameter().e);
      CoefficientTable[dir_perm[1]] = Rem(CoefficientTable[dir_perm[1]] + i, GetParameter().e);
      std::swap(dir_perm[0], dir_perm[1]);
    };

    ComplexStandardBraidUnderlying LeftMeet(const ComplexStandardBraidUnderlying &b) const;

    inline ComplexStandardBraidUnderlying RightMeet(const ComplexStandardBraidUnderlying &b) const
    {
    ...
    ComplexStandardBraidUnderlying Product(const ComplexStandardBraidUnderlying &b) const;

    // Under the assumption a <= b, a.LeftComplement(b) computes
    // The factor c such that ac = b.
    ComplexStandardBraidUnderlying LeftComplement(const ComplexStandardBraidUnderlying &b) const;

    ComplexStandardBraidUnderlying RightComplement(const ComplexStandardBraidUnderlying &b) const;
